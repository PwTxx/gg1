...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Já Foi Tarde, Não Volte Mais🤬 @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const FadhilGraphy = ["553398091185@s.whatsapp.net"]
			...
			mess = {
				...
				only: {
					...
					ownerG: '*[⚠️]Desculpe, este comando só pode ser usado pelo grupo proprietário!*',
					ownerB: '*[⚠️]Desculpe, este comando só pode ser usado pelo proprietário do bot!* ',
					premium: '*[⚠️]Desculpe, este comando só pode ser usado por usuários premium!!*',
					userB: `[ ⚠️NÃO REGISTRO⚠]\n\n️Salve *${pushname2}* Vocês ainda não são amigos *${name}* \nRegistre-se primeiro Sim por via\n\n Tipo ${prefix}daftar Tiago/18`,
					admin: '*[⚠️]Desculpe, este comando só pode ser usado por administradores de grupo!*',
			...
			const ownerNumber = [
			"553398091185@s.whatsapp.net"//GANTI NOMOR LU
			]
			premium = [
			"553398091185@s.whatsapp.net","553387192047@s.whatsapp.net"//GANTI NOMOR YAG MAU DI PREM, GABISA NYIMPEN DI DATABASE!!
			]
     ...
     const bayarLimit = (sender, amount) => {
		...
		case 'antilink':
					...
					if (args[0] == 'on') {
						...
						fs.writeFileSync('./src/antilink.json', JSON.stringify(anlink))
						reply('Modo anti-link desativado com sucesso️')
					} else {
			...
			case 'nyw':
			   ...
			   if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                 if (args.length < 1) return reply (`Parâmetro incorreto\nComando : ${prefix}daftar nome/idade/origem\n\nContoh : ${prefix}daftar Tiago/18/Brasil`)
                 reg = `{bodyslice(8)}`
                 jeneng = reg.slipt("/")[0];
                 umure = reg.split("/")[1];
                 asal = reg.split("/")[2];
                 ...
                 client.sendMessage(from, ppimg, image, { quoted: mek, caption: '*Registro bem sucedido*'})
		...
		case 'daftar':
					...
					if (isBanned) return reply(mess.only.benned)
					if (args.length < 1) return reply(`Parâmetros errados\nComando : ${prefix}daftar nome/idade\nExemplo : ${prefix}daftar Tiago/18`)
					reg = `${body.slice(8)}`
					jeneng = reg.split("/")[0];
					umure = reg.split("/")[1];
						user.push(sender)
			            fs.writeFileSync('./database/json/user.json', JSON.stringify(user))
						await costum(`\`\`\`O registro foi bem sucedido com SN: 87Y4NG4N5K4MU8U71QC4ND44NJ9\`\`\`\n\n\`\`\`Sobre ${date} ${time}\`\`\`\n\`\`\`[Nome]: ${jeneng}\`\`\`\n\`\`\`[Número]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[Idade]: ${umure} Ano\`\`\`\n\`\`\`[Origem]: ${asal}\`\`\`\n\`\`\`Para usar um bot\`\`\`\n\`\`\`Por favor\`\`\`\n\`\`\`enviar ${prefix}help/menu\`\`\`\n\`\`\`\nTotal de usuários: ${user.length} Pessoa\`\`\``, texto, TiagoGraphy, rdaftar)
					break 
			...
			case 'fadhil':
                ...
                myDays = ['Domingo','Segunda','Terça','Quarta','Quinta','Sexta','Sabádo'];
                var tgl = new Date();
                var day = tgl.getDate()
                  bulan = tgl.getMonth()
...
/*********END PISAH MENU***********/
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break
				...
				case 'owner':
				case 'creator':
                 client.sendMessage(from, {displayname: "+55 33-9809-1185", vcard: vcard}, MessageType.contact, { quoted: mek})
                 client.sendMessage(from, 'Se você quiser salvar o bate-papo, salve-o de volta:)',text, { quoted: mek} )
                 break
                 case 'fitnah':
                 if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB) 
				...
				case 'addprem':
					...
					addpremium = addpremi
					reply(`*Sucesso adicionado ${addpremium} Para Usuário Premium*\n\nFeliz com os recursos premium:)`)
					break
				case 'removeprem':
				case 'dellprem':
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'unban':
					...
					reply(`Numero wa.me/${bnnd} foi desbanido!`)
					break
				case 'block':
				 client.updatePresence(from, Presence.composing) 
...
*/
              case 'anime':
				...
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					if (!isAnime) return reply(' *Deve ativar o modo Animes* ')
					reply(mess.wait)
...
//creator
				...
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
			if (args.length < 1) return reply(`${name} O que escrever, tio??`)
			reply(mess.wait)
					tulis = body.slice(7)
				  nama = tulis.split("/")[0];
					kelas = tulis.split("/")[1];
...
//akhir kreator
		    ...
		    case 'darkjokes':
				...
				reply(mess.wait)
                 ...
                 client.sendMessage(from, darkjokes, image, {quoted: mek, caption: '\`\`\`🐊🐊\`\`\`'})
				await limitAdd(sender) 
				break  
	   case 'coli':
				client.updatePresence(from, Presence.composing) 
			...
			case 'bucinpersen':
				...
				client.sendMessage(from, 'Percent Bucin Sis: *'+rate+'*\n\nJawaban : '+ pbucin +'', text, { quoted: mek })
				await limitAdd(sender) 
				break 
		    case 'map':
			    if (isBanned) return reply(mess.only.benned)    
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					...
					if (!tipelist.includes(args[0])) return reply('Desktop | tablet | tipo móvel')
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 TIAGOBOT 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat BRYANBOT\`\`\``)
					break
                                case 'bcgc':
				...
				case 'toimg':
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					if (!isQuotedSticker) return reply('responder stickernya')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: '@tiago.lve'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender) 
					break 
	...
	case 'infocuaca':
     ...
     if (!isUser) return reply(mess.only.userB)
     if (isLimit(sender)) return reply(limits.limitend(pushname2))
     if (args.length < 1) return reply(from, 'Enviar comando *!cuaca [tempat]*\nExemplo : *!cuaca São Paulo', text)
     reply(mess.wait)
				...
				default:
                  ...
                  reply(`Maaf Kak *${pushname2}*, Command *${prefix}${command}* Tidak Terdaftar Di Dalam *${prefix}menu!*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[FADHILBOT]','Yellow'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'yellow'))
